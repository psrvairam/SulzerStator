...
//  LINE operator+(const POINT point);
  ...
  void splitStephan(LINE interline, deque<POINT> &line1, deque<POINT> &line2);
  void splitStephan2(LINE interline1,LINE interline2, deque<POINT> &line1, deque<POINT> &line2, int &istart, int &iend);
  void splitStephan3(POINT pt1,POINT pt2, deque<POINT> &line1, deque<POINT> &line2);
  ...
  void writeCtrPtsAnsys(const char *name);

  deque<POINT> discretize(int npoints, bool startend, bool reverse);
  deque<POINT> discretize2(const int imax,
...
public:
  ...
  SPLINE(const POINT &Pt1, const POINT &Pt2)
  {
    controlPt.push_back(Pt1);
    controlPt.push_back(Pt2);
    iStart = 0;
...
//  }
  ...
  POINT calcTang2D(double t) const;

  POINT calcNorm2D_new(double t) const;


  deque<POINT> discretizeSplineS(int npoints, bool startend, bool reverse, double sstart, double send);
